...
function renderAccounting() {
    ...
    // 根據目前的分頁狀態，呼叫對應的渲染函式
    if (currentAccTab === 'details')
        renderAccDetails(); // 如果是 'details' (明細) 分頁
    else if (currentAccTab === 'chart')
        renderAccChart(); // 如果是 'chart' (圖表) 分頁
    else if (currentAccTab === 'daily')
        renderAccDaily(); // 如果是 'daily' (日統計) 分頁
    else if (currentAccTab === 'accounts')
        renderAccAccounts(); // 如果是帳戶分頁，就渲染帳戶列表
}
// 切換記帳分頁的函式
function switchAccTab(tabName) {
    ...
    tabs.forEach(t => {
        ...
        if (btn)
            btn.classList.remove('active'); // 移除按鈕的 active 樣式
        if (view)
            view.style.display = 'none'; // 隱藏該分頁的內容
    });
...
function renderAccChart() {
    ...
    accountingList.forEach(item => {
        ...
        if (!monthlyData[month])
            monthlyData[month] = { income: 0, expense: 0 }; // 若該月份尚未在物件中，則初始化為 0
...
function openAccountingModal() {
    ...
    // 更新下拉選單
    if (typeof updatePaymentMethodOptions === 'function')
        updatePaymentMethodOptions();
...
function renderAccAccounts() {
    ...
    accountingList.forEach(item => {
        ...
        if (item.type === 'income') {
            ...
            if (toMethod) {
                if (balances[toMethod] === undefined)
                    balances[toMethod] = 0;
...
function updatePaymentMethodOptions() {
    ...
    if (selectTo)
        selectTo.innerHTML = optionsHtml; // 同步填入轉入選單
    // 恢復選取狀態
    if (paymentMethods.includes(currentVal))
        select.value = currentVal;
    if (selectTo && paymentMethods.includes(currentValTo))
        selectTo.value = currentValTo;
}
// 新增支付方式
