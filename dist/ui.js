...
function switchTab(tabName, addToHistory = true) {
    ...
    if (tabName === 'home') {
        if (backBtn)
            backBtn.style.display = 'none'; // 隱藏返回鍵
        if (titleEl)
            titleEl.innerText = '📅 校園王'; // 設定首頁標題
    }
    else {
        // 如果是其他內頁
        if (backBtn)
            backBtn.style.display = 'block'; // 顯示返回鍵
        // 根據頁面名稱設定標題
        let pageTitle = "校園王";
        switch (tabName) {
            case 'schedule':
    ...
    if (tabName === 'schedule') {
        ...
        // 新增這一行，確保預設顯示第一個分頁
        if (typeof switchScheduleMode === 'function')
    ...
    if (tabName === 'info')
        loadAnnouncements();
    // 如果切換到成績管理，預設顯示總覽分頁
    if (tabName === 'grade-manager' && typeof switchGradeTab === 'function')
        switchGradeTab('dashboard');
    // 如果切換到記帳頁，預設顯示摘要分頁
    if (tabName === 'accounting') {
        if (typeof switchAccTab === 'function')
            switchAccTab('summary');
