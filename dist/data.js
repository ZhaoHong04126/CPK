...
function refreshUI() {
    ...
    if (typeof updateExamSubjectOptions === 'function')
        updateExamSubjectOptions();
    switchDay(currentDay); // 重新渲染「本日課程」與「週課表」
    loadGrades(); // 重新計算並渲染成績列表與平均分數
    // 安全地呼叫各個模組的渲染函式 (如果該模組已載入)
    if (typeof renderRegularExams === 'function')
        renderRegularExams(); // 平常考列表
    if (typeof renderMidtermExams === 'function')
        renderMidtermExams(); // 段考列表
    if (typeof renderCalendar === 'function')
        renderCalendar(); // 行事曆
    if (typeof renderWeeklyTable === 'function')
        renderWeeklyTable(); // 完整週課表網格
    if (typeof renderAnalysis === 'function')
        renderAnalysis(); // 成績趨勢圖表分析
    // --- 設定頁面數值回填 ---
    // 取得畢業學分目標的輸入框元素
    const targetInput = document.getElementById('setting-grad-target');
    // 若元素存在，將目前的設定值 (graduationTarget) 填入輸入框
    if (targetInput)
        targetInput.value = graduationTarget;
    if (typeof renderCategorySettingsInputs === 'function')
        renderCategorySettingsInputs(); // 學分分類設定輸入框
    if (typeof renderCreditSettings === 'function')
        renderCreditSettings(); // 學分設定介面 (高中/大學)
    if (typeof renderAccounting === 'function')
        renderAccounting(); // 記帳介面
    if (typeof renderNotes === 'function')
        renderNotes(); // 筆記列表
    if (typeof renderAnniversaries === 'function')
        renderAnniversaries(); // 紀念日列表
    if (typeof renderSemesterSettings === 'function')
        renderSemesterSettings(); // 學期日期設定介面
    if (typeof renderLottery === 'function')
        renderLottery(); // 重新渲染籤筒
    if (typeof renderNotificationApp === 'function')
        renderNotificationApp(); // 如果有定義通知
    if (typeof renderHomework === 'function')
        renderHomework(); // 重新渲染作業列表
    // [新增] 確保成績輸入視窗的「分類下拉選單」是根據使用者的設定產生的
    if (typeof updateGradeCategoryOptions === 'function')
        updateGradeCategoryOptions();
    // 更新頂部導航列的名稱
    const nameDisplay = document.getElementById('user-name-display');
    if (nameDisplay)
        nameDisplay.innerText = userTitle;
    // 更新設定頁面的目前狀態
    const settingName = document.getElementById('setting-user-title');
    if (settingName)
        settingName.innerText = userTitle;
}
// 載入指定學期的資料到全域變數
...
function updateCategorySettings(category, type, value) {
    ...
    if (typeof categoryTargets[category] === 'object') {
        if (type === '必修')
            categoryTargets[category]['必修'] = val;
        if (type === '選修')
            categoryTargets[category]['選修'] = val;
    }
    else {
